...
- **Diagram checks and runtime:** Stabilized Mermaid geometry across Windows and Linux with Arial-compatible metrics, added a GitHub Actions-only Puppeteer configuration so freshness checks can launch Chromium on hosted Ubuntu runners, and moved sanitized SVG enhancement to the client entry so it survives client navigation and reconnects.
